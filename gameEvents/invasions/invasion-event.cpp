                            ...
                            const eCityId cid)
{
    ...
    ePathFinder p([](eTileBase *const t)
                  { return t->hasWater(); }, [&](eTileBase *const t)
                  {
    ...
    const eInvasionResult result)
{
    ...
    auto &board = *gameBoard();
    const int amount = result == eInvasionResult::invaderWonOrPlayerSurrendered ? kInvaderWonAttitudeRestore : kInvaderDefeatedAttitudeRestore;
    board.changeCityAttitude(mCity, amount, board.personPlayer());
}
