...
TraderAction::TraderAction(eCharacter* const c) :
    eActionWithComeback(c, eCharActionType::traderAction) {}

bool TraderAction::decide() {
...
void TraderAction::setTradePost(TradePost* const tp) {
    mTradePost = tp;
}

void TraderAction::setUnpackBuilding(eBuilding* const b) {
    mUnpackBuilding = b;
}
...
void TraderAction::goToTradePost() {
    const auto c = character();

    const stdptr<TraderAction> tptr(this);
