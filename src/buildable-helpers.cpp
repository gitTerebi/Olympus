...
bool BuildableHelpers::canBuildFisheryTR(const eTileBase* const t) {
    if(!t) return false;
...
bool BuildableHelpers::canBuildFisheryBR(const eTileBase* const t) {
    if(!t) return false;
    if(!tileDry(t)) return false;
...
bool BuildableHelpers::canBuildFisheryBL(const eTileBase* const t) {
    if(!t) return false;
...
bool BuildableHelpers::canBuildFisheryTL(const eTileBase* const t) {
    if(!t) return false;
    if(!tileWater(t)) return false;
