...
bool gBuild(const AIBuilding& b,
    ...
    case eBuildingType::warehouse: {
        const auto bc = [boardPtr, cid, b]() {
            const auto w = e::make_shared<Warehouse>(*boardPtr, cid);
    ...
    case eBuildingType::tradePost: {
        ...
        if(b.feTradePostType == eTradePostType::post) return tpr;
        if(!tpr) return false;
        const auto bcp = [boardPtr, cid, b, tpPtr]() {
            const auto& wboard = boardPtr->world();
            const auto wc = wboard.cityWithId(b.fTradingPartner);
    ...
    case eBuildingType::templeZeus: {
        ...
        const int rotateId = rotate ? 1 : 0;
        return board.buildSanctuary(minX, maxX, minY, maxY, b.fType,
                                    rotateId, cid, pid, editorDisplay);
    } break;
    default:
        break;
    }
...
bool AIDistrict::sBuild(GameBoard& board,
    ...
    if(b.fType == eBuildingType::horseRanch) {
        ...
        if(!r) return false;
              ...
              b.feTradePostType == eTradePostType::pier) {
        const bool r = processRect(b.fOtherRect);
        if(!r) return false;
    }
...
bool AIDistrict::road(int& x, int& y) const {
    for(const auto& b : fBuildings) {
...
void AIDistrict::addBuilding(const AIBuilding& a) {
