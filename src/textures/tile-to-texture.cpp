...
std::shared_ptr<Texture> TileToTexture::get(eTile* const tile,
    ...
    if(drawElev) {
        ...
        if(!halfSlope) {
            ...
            int dx;
            int dy;
            eTile* const under = VaryingSizeTex::hiddenByNeighbour(tile, dir, dx, dy);
            if(under) {
...
//                const bool tw = t ? t->walkableElev() : false;
    ...
    case eTerrain::beach: {
        const int toDryId = BeachToDry::get(tile, dir);
        if(toDryId != -1) {
    ...
    case eTerrain::lava: {
        const int toDryId = LavaToDry::get(tile, dir);
        if(toDryId != -1) {
    ...
    case eTerrain::marsh: {
        const int toDryId = MarshToDry::get(tile, dir);
        if(toDryId != -1) {
    ...
    case eTerrain::fertile: {
        const auto id = FertileToDry::get(tile, dir);
        const int scrubCount = 12;
        const int scrub = tile->scrubId(scrubCount) - 1;
        switch(id) {
        case FertileToDryId::none: {
            if(scrub == -1) {
    ...
    case eTerrain::forest: {
        ...
        if(rainforest) GameTextures::loadPoseidonTrees();
        const auto id = ForestToDry::get(tile, dir);
        switch(id) {
        case ForestToDryId::none: {
            const auto& coll = rainforest ? textures.fPoseidonForestTerrainTexs :
                                              textures.fForestTerrainTexs;
            const int texId = seed % coll.size();
            return coll.getTexture(texId);
        } break;
        case ForestToDryId::somewhere: {
    ...
    case eTerrain::marble: {
        return MarbleTile::get(tile, textures, dir);
    } break;
