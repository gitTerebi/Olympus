...
std::vector<eTile*> GameWidget::agoraBuildPlaceBR(
        eTile* const tile, const eCityId cid, const ePlayerId pid) const {
    return ::agoraBuildPlaceBR(mBoard, mEditorMode, tile, cid, pid);
}
std::vector<eTile*> GameWidget::agoraBuildPlaceTL(
        eTile* const tile, const eCityId cid, const ePlayerId pid) const {
    return ::agoraBuildPlaceTL(mBoard, mEditorMode, tile, cid, pid);
}
std::vector<eTile*> GameWidget::agoraBuildPlaceBL(
        eTile* const tile, const eCityId cid, const ePlayerId pid) const {
    return ::agoraBuildPlaceBL(mBoard, mEditorMode, tile, cid, pid);
}
std::vector<eTile*> GameWidget::agoraBuildPlaceTR(
        eTile* const tile, const eCityId cid, const ePlayerId pid) const {
    return ::agoraBuildPlaceTR(mBoard, mEditorMode, tile, cid, pid);
}
...
GameWidget::eApply GameWidget::editFunc() {
...
bool GameWidget::buildStampAgora(const std::vector<eTile*>& tiles,
...
bool GameWidget::buildMouseRelease() {
    return buildModeAt(mGm->mode(), mHoverTX, mHoverTY, mPressedTX, mPressedTY);
}

bool GameWidget::buildModeAt(const eBuildingMode mode,
...
//        if(brushType != eBrushType::apply) return true;
        ...
        case eBuildingMode::pier: {
            ...
            if(c) {
                ...
                if(accessToSea) {
                    ...
                    const auto tp = e::make_shared<TradePost>(
        ...
        case eBuildingMode::warehouse: {
            stdsptr<Warehouse> wh;
            r = mBoard->build(hoverTX, hoverTY, 3, 3, cid, pid, mEditorMode,
                  [this, &wh]() {
                      auto w = e::make_shared<Warehouse>(*mBoard, mViewedCityId);
                      wh = w;
                      return w;
                  });
        ...
        case eBuildingMode::tradePost: {
            ...
            r = mBoard->build(hoverTX, hoverTY, 4, 4, cid, pid, mEditorMode,
                  [this, ct]() {
                const auto tp = e::make_shared<TradePost>(*mBoard, *ct, mViewedCityId);
                return tp;
            });
        } break;
