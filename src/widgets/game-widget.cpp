...
GameWidgetSettings GameWidget::settings() const
{
    GameWidgetSettings r;
    r.fPaused = mPaused;
    r.fSpeedId = mSpeedId;
    r.fSpeed = mSpeed;
...
void GameWidget::setSettings(const GameWidgetSettings &s)
{
...
void GameWidget::rebuildGameMenu()
{
    if(mGm) {
        mGm->clearMode();
        removeWidget(mGm);
...
void GameWidget::initialize()
{
    ...
    if (mEditorMode)
    {
        ...
        condButton->setPressAction([this]()
                                   {
            const auto condsMenu = new DistrictConditionsWidget(window());
...
void GameWidget::toggleViewMode(const eViewMode m)
{
    setViewMode(mViewMode == m ? eViewMode::defaultView : m);
}

void GameWidget::mapDimensions(int &mdx, int &mdy) const
{
...
void GameWidget::viewBoxSize(double &fx, double &fy) const
{
    int mdx;
    int mdy;
...
void GameWidget::viewFraction(const double fx, const double fy)
{
    int mdx;
    int mdy;
    mapDimensions(mdx, mdy);
...
void GameWidget::viewTile(eTile *const tile)
{
    if (!tile)
        return;
    int mdx;
...
eTile *GameWidget::viewedTile() const
{
    double fx;
    double fy;
...
bool GameWidget::tileVisible(eTile *const tile) const
{
    if (!tile)
        return false;
...
eCityId GameWidget::viewedCity() const
{
    return mViewedCityId;
}

void GameWidget::showBuyCity(const eCityId cid)
{
...
int GameWidget::rotationId() const
{
    return mRotateFrame / gRotateFrames;
}

int GameWidget::hippodromeId() const
{
...
void GameWidget::updateHippodromeIds()
{
...
void GameWidget::showTip(const ePlayerCityTarget &target,
                          const std::string &tip)
{
...
void GameWidget::updateTipPositions()
{
    const int p = padding();
    int y;
    if (mPausedLabel)
...
bool GameWidget::inErase(const SDL_Rect &rect)
{
...
eBuilding* GameWidget::eraseBuildingAt(const int tx, const int ty) const
{
    if(const auto b = mBoard->buildingAt(tx, ty)) return b;
    return nullptr;
}

eTile* GameWidget::eraseParkParentTileAt(const int tx, const int ty) const
{
...
bool GameWidget::inErase(eAgoraBase *const a)
{
    if(!a) return false;
...
bool GameWidget::inErase(eBuilding *const b)
{
    if (!b)
        return false;
    const auto mode = mGm->mode();
    ...
    else if (const auto tp = dynamic_cast<TradePost *>(b))
    {
...
bool GameWidget::inRepair(const int tx, const int ty)
{
    const auto mode = mGm->mode();
...
bool GameWidget::inPatrolBuildingHover(eBuilding *const b)
{
    const auto mode = mGm->mode();
...
void GameWidget::setSpeedId(const int id)
{
    mSpeedId = std::clamp(id, 0, sMaxSpeedId);
    mSpeed = sSpeeds[mSpeedId];
}

void GameWidget::showSpeedLabel()
{
...
bool GameWidget::keyPressEvent(const eKeyPressEvent &e)
{
    if (mLocked)
        return true;
...
bool GameWidget::keyReleaseEvent(const eKeyPressEvent &e)
{
    updateSmoothScrollKey(e.key(), false);
    return true;
}

bool GameWidget::updateSmoothScrollKey(const SDL_Scancode k,
                                        const bool pressed)
{
    const auto &hotkeys = window()->settings();
...
bool GameWidget::smoothScrollKeyPressed(const SDL_Scancode k) const
{
...
void GameWidget::updateSmoothScrollKeysPressed()
{
    if(mLocked) return;
    if(mStampTemplateNameDialogOpen) {
        mSmoothScrollX = 0;
...
void GameWidget::updateKeyScrollSpeed(const int speed)
{
    mKeyScrollSpeed = speed * 5;
}

void GameWidget::stopSmoothScroll()
{
    mSmoothScrollX = 0;
    mSmoothScrollY = 0;
    mSmoothScrollFineX = 0.;
...
bool GameWidget::selectedPlayerBanners() const
{
...
bool GameWidget::mouseReleaseEvent(const eMouseEvent &e)
{
...
bool GameWidget::mouseWheelEvent(const eMouseWheelEvent &e)
{
    if (mLocked)
        return true;
    const int cityX = windowToCityX(e.x());
...
void GameWidget::cancelStampTemplateCreate()
{
...
void GameWidget::updateStampTemplatePanel()
{
...
void GameWidget::showStampTemplateNameDialog()
{
...
void GameWidget::saveStampTemplate(const std::string& name)
{
...
std::vector<eStampElement> GameWidget::stampTemplateElements(
...
void GameWidget::selectHoveredBuildingMode()
{
...
void GameWidget::setDX(const int dx)
{
    const int oldDX = mDX;
    mDX = dx;
    clampViewBox();
...
void GameWidget::setDY(const int dy)
{
    const int oldDY = mDY;
    mDY = dy;
    clampViewBox();
...
void GameWidget::viewBookmark(const int id)
{
    if (!mBoard)
        return;
...
void GameWidget::updateTopBottomAltitude()
{
    if (!mBoard)
        return;
    const auto dir = mBoard->direction();
...
void GameWidget::updateMinMaxAltitude()
{
    mBoard->minMaxAltitude(mMinAltitude, mMaxAltitude);
}

void GameWidget::updateMaps(const bool totalUpdate)
{
    const auto mm = mGm->miniMap();
    const auto mma = mAm->miniMap();
    const auto mmt = mTem->miniMap();
...
void GameWidget::updateMaps(const std::vector<eTile *> &tiles)
{
    const auto mm = mGm->miniMap();
    const auto mma = mAm->miniMap();
    const auto mmt = mTem->miniMap();
...
void GameWidget::updateCitiesOnBoard()
{
...
void GameWidget::updateViewBoxSize()
{
    if (!mBoard || !mGm)
        return;
    double fx;
...
void GameWidget::setWorldDirection(const eWorldDirection dir)
{
    if (!mBoard)
        return;
...
void GameWidget::centerDialog(eWidget *const d)
{
    d->setY((height() - d->height() - mTopBar->height()) / 2);
    d->setX((width() - d->width() - mGm->width()) / 2);
}

void GameWidget::openDialog(eWidget *const d)
{
    addWidget(d);
    centerDialog(d);
    window()->execDialog(d);
...
void GameWidget::updateRequestButtons()
{
    mGm->updateRequestButtons();
}

void GameWidget::setMessageListWidget(eMessageListWidget* const w)
{
    mMsgListWidget = w;
